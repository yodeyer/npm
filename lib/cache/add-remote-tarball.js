...
function fetchAndShaCheck (u, tmp, shasum, auth, cb) {
  npm.registry.fetch(u, { auth : auth }, function (er, response) {
    if (er) {
      ...
      if (er.request && er.request.headers) {
        log.verbose('fetch request headers', er.request.headers)
      }
      if (er.response && er.response.headers) {
        log.verbose('fetch response headers', er.response.headers)
      }
