...
function addRemoteGit (uri, _cb) {
  ...
  if (parsed) {
    ...
    tryEach([cloneWithHTTPS, cloneWithSSH, cloneWithGitProto], [from, parsed], cb)
  } else {
...
// tryEach(toTry, args, cb)
// Takes a list of async functions to try in turn– this is short circuited
// when one succeeds.  If none succeed then cb is called with the final
// failing error.
function tryEach (toTry, args, whenDone) {
  args.push(tryNext)
  return tryNext(true)

  function tryNext(err) {
    if (toTry.length === 0 || !err) {
      return whenDone.apply(null, arguments)
    }
    return toTry.shift().apply(null, args)
  }
}

function createBadProtoError (from, proto) {
  var err = new Error(from + ' can not be cloned via ' + proto)
  err.code = 'EBADGITPROTO'
  err.from = from
  err.proto = proto
  return err
}

function cloneWithGitProto (from, hostedInfo, cb) {
  var gitURL = hostedInfo.git()
  if (!gitURL) return cb(createBadProtoError(from, 'git'))

  log.silly('tryGitProto', 'attempting to clone', gitURL)
  tryClone(from, gitURL, true, cb)
}

function cloneWithHTTPS (from, hostedInfo, cb) {
  var httpsURL = hostedInfo.https()
  if (!httpsURL) return cb(createBadProtoError(from, 'https'))

  log.silly('tryHTTPS', 'attempting to clone', httpsURL)
  tryClone(from, httpsURL, true, cb)
}

function cloneWithSSH (from, hostedInfo, cb) {
  var sshURL = hostedInfo.ssh()
  if (!sshURL) return cb(createBadProtoError(from, 'ssh'))
