...
exports.loadRequestedDeps = function (args, tree, saveToDependencies, log, next) {
  ...
  asyncMap(args, function (pkg, done) {
    var depLoaded = andAddParentToErrors(tree, andLoadDepsForChild(loadDeps, done))
...
function andLoadDepsForChild (load, done) {
  return function (err, child, childLog) {
    if (err || !child) {
      return done(err, child, childLog)
    }
    load(child, childLog, done)
  }
}
...
function loadDeps (tree, log, next) {
  ...
  asyncMap(Object.keys(tree.package.dependencies), function (dep, done) {
    ...
    addDependency(dep, version, tree, log.newGroup('loadDep:' + dep),
      andHandleOptionalErrors(log, tree, dep, andLoadDepsForChild(loadDeps, done)))
  }, andFinishTracker(log, next))
}
...
exports.loadDevDeps = function (tree, log, next) {
  ...
  asyncMap(Object.keys(tree.package.devDependencies), function (dep, done) {
    ...
    addDependency(dep, tree.package.devDependencies[dep], tree, logGroup, andLoadDepsForChild(loadDeps, done))
  }, andFinishTracker(log, next))
}
...
exports.loadExtraneous = function loadExtraneous (tree, log, next) {
  ...
  function loadExtraneous (tree, log, next) {
    ...
    asyncMap(tree.children.filter(function (child) { return !child.loaded }), function (child, done) {
      resolveWithExistingModule(child, tree, log, andLoadDepsForChild(loadExtraneous, done))
    }, andFinishTracker(log, next))
  }
...
exports.loadExtraneous.andResolveDeps = function (tree, log, next) {
  validate('OOF', arguments)
  asyncMap(tree.children.filter(function (child) { return !child.loaded }), function (child, done) {
    resolveWithExistingModule(child, tree, log, andLoadDepsForChild(loadDeps, done))
  }, andFinishTracker(log, next))
}

var addDependency = queue(function (name, versionSpec, tree, log, done) {
...
}).unique().limit(npm.config.get('fetch-size-max'))
