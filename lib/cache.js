  ...
  realizePackageSpecifier(spec, where, function (err, p) {
    ...
    switch (p.type) {
      ...
      case "remote":
        ...
        mapToRegistry(spec, npm.config, function (err, uri, auth, registry) {
          if (err) return cb(err)

          if (url.parse(registry).hostname !== url.parse(p.spec).hostname) {
            log.verbose('cache', p.spec, 'is not on the same domain as', registry)
            log.verbose('cache', 'not sending bearer token with requests')
            auth.token = null
          }
